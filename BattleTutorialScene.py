...
class BattleTutorialScene(SceneBase):
	def __init__(self):
		super().__init__()
		self.image = None
		...
		self.hint = GameObject.GameObject()
		self.hintFont = None
	...
	def init(self):
		...
		self.hintFont = ResourceManager.instance.getResourceHandler("defaultFont")
		self.hint.init(self.hintFont.render("Hint: Summon your servants to defeat enemy!", True, (255, 255, 0)), (30, 30), (10, 10))
					...
					for (index, (pos, cUI)) in self.player1Hand.characUIDict.items():
						if cUI.pointCollide(event.pos):
							if self.status.board.characDict[0][1].state < 2:
								...
								self.hint.changeAvatar(self.hintFont.render("Hint: Drag into blue blocks to summon.", True, (255, 255, 0)))
							break
					...
					if self.movingObject == None:
						for (index, (pos, cUI)) in self.boardUI.characUIDict.items():
							if cUI.pointCollide(event.pos):
								...
								self.hint.changeAvatar(self.hintFont.render("Hint: Drag into blue blocks to move.", True, (255, 255, 0)))
								break
						...
						# 		self.movingObject.position = list(self.player2Hand.getPosOnScreen(self.movingObjectInfo[1])[:])
							...
							if self._cursorFocus == 3: # board: move / attack
								...
								if UIpos[0] == UItargetPos[0] and UIpos[1] == UItargetPos[1]:
									...
									self.hint.changeAvatar(self.hintFont.render("Hint: Click enemy to attack, click himself to standby.", True, (255, 255, 0)))
								...
								print("UIlayer: illegal operation")
								self.movingObject.position = list(self.boardUI.getPosOnScreen(self.movingObjectInfo[1])[:])
								self.hint.changeAvatar(self.hintFont.render("Hint: Click turn end if nothing else to do.", True, (255, 255, 0)))
	...
	def draw(self):
		...
		self.hint._drawProto(self.screen)
